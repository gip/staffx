---
id: a.p.api.features.summary.v1
nodeId: p.api
concern: Features
type: Summary
language: en
...
This summary captures the current operational role of API Service Process (Process) for concern Features. It is intended for rebuild teams to quickly understand what must be preserved before validating detailed specifications.

Current scope: API Service Process participates in 5 outbound and 3 inbound graph connections. The node runs with ownership=first_party and boundary=internal. These constraints define who can modify this node and which trust boundaries are crossed by its traffic.
