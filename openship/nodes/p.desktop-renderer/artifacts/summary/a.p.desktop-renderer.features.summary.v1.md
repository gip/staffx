---
id: a.p.desktop-renderer.features.summary.v1
nodeId: p.desktop-renderer
concern: Features
type: Summary
language: en
---

This summary captures the current operational role of Desktop Renderer Process (Process) for concern Features. It is intended for rebuild teams to quickly understand what must be preserved before validating detailed specifications.

Current scope: Desktop Renderer Process participates in 3 outbound and 1 inbound graph connections. The node runs with ownership=first_party and boundary=external. These constraints define who can modify this node and which trust boundaries are crossed by its traffic.
