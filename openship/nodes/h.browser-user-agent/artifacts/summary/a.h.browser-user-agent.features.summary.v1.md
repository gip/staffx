---
id: a.h.browser-user-agent.features.summary.v1
nodeId: h.browser-user-agent
concern: Features
type: Summary
language: en
---

This summary captures the current operational role of Browser User Agent (Host) for concern Features. It is intended for rebuild teams to quickly understand what must be preserved before validating detailed specifications.

Current scope: Browser User Agent participates in 0 outbound and 0 inbound graph connections. The node runs with ownership=third_party and boundary=external. These constraints define who can modify this node and which trust boundaries are crossed by its traffic.
